...
class nodeActions extends apActions
{
  ...
  // add company condition to the node index -- Lennox
  public function executeIndex(sfWebRequest $request)
  {
  	$user_id = $this->getUser()->getGuardUser()->getId();
  	$company_id = Doctrine::getTable('Company')->findOneBy('user_id', $user_id)->getId();
  	$this->nodes = Doctrine::getTable('Node')->getNodesByCompany($company_id);
  }
  ...
  public function executeShow(sfWebRequest $request)
  {
    ...
    $user_id = $this->getUser()->getGuardUser()->getId();
    $company_id = Doctrine::getTable('Company')->findOneBy('user_id', $user_id)->getId();
    if ($this->node->getCompanyId() != $company_id)
    {
    	$this->node = null;
    }
  ...
  public function executeCreate(sfWebRequest $request)
  {
    ...
    $user_id = $this->getUser()->getGuardUser()->getId();
    $params['company_id'] = Doctrine::getTable('Company')->findOneBy('user_id', $user_id)->getId();
    
    $files = $request->getFiles($this->form->getValue('com_image'));
    // And bind it again
  ...
  protected function processForm(sfWebRequest $request, sfForm $form)
  {
    ...
    if ($form->isValid())
    {
      $notice = $form->getObject()->isNew() ? '热点创建成功！' : '信息更新成功！';
